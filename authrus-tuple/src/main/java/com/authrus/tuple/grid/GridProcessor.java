...
import com.authrus.tuple.queue.ElementConsumer;
import com.authrus.tuple.queue.ElementListener;
import com.authrus.tuple.subscribe.SubscriptionListener;

class GridProcessor implements TransportProcessor {

   private final SubscriptionListener listener;
   private final ChangeSubscriber subscriber;
   private final AtomicBoolean active;
   private final FrameTracer tracer; 
   private final Reactor reactor;

   public GridProcessor(ChangeSubscriber subscriber, SubscriptionListener listener, FrameTracer tracer, Reactor reactor) {
   ...
   public void process(Transport transport) throws IOException {
      SocketChannel connection = transport.getChannel();
      Socket socket = connection.socket();
      SocketAddress address = socket.getRemoteSocketAddress(); // for Android!
      ...
      if(socket.isConnected() && active.get()) {
         ...
         FrameAdapter adapter = new FrameAdapter(encoder, tracer, channel, location);
   ...
   private class FrameAdapter implements FrameListener {
      ...
      public FrameAdapter(FrameEncoder encoder, FrameTracer tracer, Channel channel, String address) {
