...
public class QueueServer {
   ...
   public QueueServer(InsertSubscriber subscriber, SubscriptionListener listener, FrameTracer tracer, int port) throws IOException {
      this.executor = new ConcurrentExecutor(FrameCollector.class, 10);
      this.reactor = new ExecutorReactor(executor);
      this.processor = new QueueProcessor(subscriber, listener, tracer, reactor);
