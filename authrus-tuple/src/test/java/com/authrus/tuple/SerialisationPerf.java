...
public final class SerialisationPerf
{
    ...
    private static final PerformanceTestCase[] testCases =
    {
       ...
       new PerformanceTestCase("DataBuffer", REPETITIONS, ITEM)
       {
           ...
           {
              ByteBuffer source = buffer.extract();
              DataReader reader = new ByteBufferReader(source);
              
              ObjectToBeSerialised object = null;
               for (int i = 0; i < REPETITIONS; i++)
               {
                   source.position(0);
                   object = ObjectToBeSerialised.read(reader);
               }
       ...
       new PerformanceTestCase("Streams", REPETITIONS, ITEM)
       {
           byte[] data = new byte[1024];

           public void testWrite(ObjectToBeSerialised item) throws Exception
           {
              ByteArrayOutputStream output = new DirectByteArrayOutputStream(data);
           ...
           public ObjectToBeSerialised testRead() throws Exception
           {
              ...
              input.mark(data.length);
              
               for (int i = 0; i < REPETITIONS; i++)
               {
                   input.reset();
                   object = ObjectToBeSerialised.read(stream);
               }
...
class ObjectToBeSerialised implements Serializable
{
    ...
    public void write(final DataOutputStream byteBuffer) throws IOException
    {
        byteBuffer.writeLong(sourceId);
        byteBuffer.writeBoolean(special);
        byteBuffer.writeInt(orderCode);
        byteBuffer.writeInt(priority);
 
        byteBuffer.writeInt(prices.length);
        for (final double price : prices)
        {
            byteBuffer.writeDouble(price);
        }
 
        byteBuffer.writeInt(quantities.length);
        for (final long quantity : quantities)
        {
            byteBuffer.writeLong(quantity);
        }
        byteBuffer.flush();
    }
 
    public static ObjectToBeSerialised read(final DataInputStream byteBuffer) throws IOException
    {
        final long sourceId = byteBuffer.readLong();
        final boolean special = byteBuffer.readBoolean();
        final int orderCode = byteBuffer.readInt();
        final int priority = byteBuffer.readInt();
 
        final int pricesSize = byteBuffer.readInt();
        final double[] prices = new double[pricesSize];
        for (int i = 0; i < pricesSize; i++)
        {
            prices[i] = byteBuffer.readDouble();
        }
 
        final int quantitiesSize = byteBuffer.readInt();
        final long[] quantities = new long[quantitiesSize];
        for (int i = 0; i < quantitiesSize; i++)
        {
            quantities[i] = byteBuffer.readLong();
        }
    ...
    public void write(final DataWriter byteBuffer) throws IOException
    {
        byteBuffer.writeLong(sourceId);
        byteBuffer.writeBoolean(special);
        byteBuffer.writeInt(orderCode);
    ...
    public static ObjectToBeSerialised read(final DataReader byteBuffer) throws IOException
    {
        final long sourceId = byteBuffer.readLong();
        final boolean special = byteBuffer.readBoolean();
        final int orderCode = byteBuffer.readInt();
