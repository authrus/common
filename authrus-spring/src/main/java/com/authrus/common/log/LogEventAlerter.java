...
import com.authrus.common.mail.MailClient;
import com.authrus.common.mail.MailMessage;
...
public class LogEventAlerter extends LogEventFilter {
   ...
   @ManagedOperationParameters({ @ManagedOperationParameter(name = "repeatCount", description = "Threshold for repeat events") })
      ...
      try {
         ...
         if (eventFilter != null) {
            ...
            if (eventCount >= eventThreshold) {
               MailMessage alertMessage = mailMessage.getMessage();
               String logHistory = logEvents.toString();

               alertMessage.setBody(logHistory);
               alertCount.getAndSet(0);
               logEvents.setLength(0);
               mailClient.sendMessage(alertMessage);
            }
         }
      } catch (Throwable e) {
